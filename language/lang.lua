...
do
    for i,line in pairs(file_table) do
        ...
        :gsub(":{", ":{;")
        :gsub("break%W","_NV_break")
        :gsub("elif", "elseif")
        :gsub("repeat%W","_NVrepeat")
        :gsub("until%W","_NVuntil")
        :gsub("do%W","_NVdo")
        :gsub("end%W","_NVend")
        :gsub("then%W","_NVthen")
        :gsub("local%W","_NVlocal")
        :gsub("let%W","local")
        :gsub("return%W","_NVreturn")
        :gsub("->"," return ")
        :gsub("(%W)f%(",function(str) return str:sub(1,1).."\\(" end)
